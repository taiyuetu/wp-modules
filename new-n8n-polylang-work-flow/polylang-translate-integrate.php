<?php 
//add_action('admin_init', 'bulk_assign_product_taxonomy');

function bulk_assign_product_taxonomy()
{

    // ── CONFIG ──────────────────────────────────────────────────────
    $taxonomy = 'product_category'; // or 'product_tag', custom taxonomy
    $term_slug = 'wheel-hub-assembly'; // term to assign
    $batch = 200; // products per batch (lower on slow hosts)
    // ────────────────────────────────────────────────────────────────

    if (!isset($_GET['run_bulk_taxonomy']))
        return;

    $term = get_term_by('slug', $term_slug, $taxonomy);
    if (!$term) {
        wp_die("Term '{$term_slug}' not found in '{$taxonomy}'.");
    }

    $offset = (int)get_transient('bulk_tax_offset');
    ...
    if (empty($ids)) {
        delete_transient('bulk_tax_offset');
        wp_die('✅ Done! All products assigned.');
    }

    foreach ($ids as $id) {
        // Use false to APPEND, true to REPLACE all existing terms
        wp_set_object_terms($id, $term->term_id, $taxonomy, false);
    }

    set_transient('bulk_tax_offset', $offset + $batch, HOUR_IN_SECONDS);

    wp_die("✅ Batch done: processed IDs {$offset}–" . ($offset + $batch) . '. 
        <a href="?run_bulk_taxonomy=1">▶ Run next batch</a>');
}


/**
 * Polylang Bulk Meta Sync
 * Source of truth: default key (e.g. product_gallery)
 * Direction: default key → push to all language-specific keys (suffix pattern)
 */

// ─────────────────────────────────────────────────────────────────────────────
// CONFIGURATION — list only your DEFAULT keys here (no language suffixes)
// ─────────────────────────────────────────────────────────────────────────────

function pll_sync_get_default_keys(): array
{
    return [
        'product_gallery',
        'product_price',
        ...
        'key_features',
        'drawing'
        // Add every default meta key you want synced.
        // The language-specific keys are auto-derived: key_en, key_ru, etc.
    ];
}

// ─────────────────────────────────────────────────────────────────────────────
// CORE ENGINE
// Reads the default key value from the DEFAULT language post,
// then writes key_{lang} on every translated post.
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Sync one post's default meta keys → all its language-specific counterparts.
 *
 * @param int    $source_post_id  The post ID that holds the default key values.
 * @param string $default_lang    The language slug of the source post (e.g. 'en').
 */
function pll_sync_push_default_to_lang_keys(int $source_post_id, string $default_lang): void
{
    ...
    // All translations of this post: [ 'en' => 101, 'ru' => 102, 'fr' => 103 ]
    ...
    $default_keys = pll_sync_get_default_keys();

    foreach ($default_keys as $default_key) {

        // Read value from the SOURCE (default language) post.
        $value = get_post_meta($source_post_id, $default_key, true);

        // Nothing stored — nothing to push.
        if ($value === '' || $value === false) {
            continue;
        }

        // Push to every translated post using the lang-specific key.
        foreach ($translations as $lang => $translated_post_id) {

            $lang_key = $default_key . '_' . $lang; // e.g. product_gallery_en

            update_post_meta((int)$translated_post_id, $lang_key, $value);
        }
    }
}


function bulk_update_product_image_alt_text()
{
    // Only run once — remove after execution
    if (!isset($_GET['run_alt_fix']))
        return;

    // ⬇️ Replace 'your_post_type' with your actual custom post type slug
    $posts = get_posts([
        'post_type' => 'product',
        'posts_per_page' => -1,
        'post_status' => 'publish',
        ...
        if ($thumbnail_id) {
            ...
            if (empty($current_alt)) {
                update_post_meta(
                    ...
                    'Wheel Hub Bearing ' . $post->post_title // Uses the post title as alt text
                );
                $updated++;
            } else {
                $skipped++;
    ...
    echo "Done! Updated: {$updated} | Skipped (already had alt text): {$skipped}";
    exit;
}
//add_action( 'init', 'bulk_update_product_image_alt_text' );
 ...
 * Changelog:
 *   v1.4.3 — 2026-03-18
 *     - FIX: plt_get_untranslated_posts() was treating empty Polylang stub posts as
 *       "already translated" and returning posts: []. When a language subdomain or
 *       directory is configured in Polylang (e.g. en.webclassmate.com), Polylang
 *       auto-creates empty published stub posts for every source post. These stubs
 *       have no title and no content but are 'publish' status, so the previous check
 *       (post exists + is published → skip) skipped them all. Fixed by adding a
 *       content check: a translation is only considered complete if the linked post
 *       has a non-empty title OR non-empty content.
 *
 *   v1.4.2 — 2026-03-18
 *     - FIX: Route arg was registered as 'lang' but plt_get_untranslated_posts() read
 *       'target_lang'. Renamed the registered arg to 'target_lang' so WordPress
 *       sanitize_callback fires correctly and the param is not silently dropped.
 *     - FIX: Posts whose Polylang translation ID exists but the translated post was
 *       deleted (stale link) were incorrectly treated as "already translated", causing
 *       posts[] to be empty. Now verifies the linked post exists AND is published.
 *
...
function plt_get_untranslated_posts(WP_REST_Request $request)
{
    ...
    $target_lang = $request->get_param('target_lang'); // ← renamed from 'lang'
    ...
    $post_ids = get_posts([
        ...
        'post_status' => 'publish',
        'posts_per_page' => -1,
        'lang' => '', // Fetch posts in all languages
        'orderby' => 'ID',
        'order' => 'DESC', // Check newest posts first
        'fields' => 'ids',
    ]);
    ...
    foreach ($post_ids as $pid) {
        ...
        // Find the oldest post in the translation group to act as the single source
        $min_id = $pid;
        if (!empty($translations)) {
        ...
        // If this post is not the oldest in its translation group, skip it to avoid duplicate translation jobs
        if ($pid != $min_id) {
            continue;
        }
        ...
        foreach ($check_langs as $lang) {
            ...
            // A non-zero ID in the Polylang map is NOT enough:
            // the translated post may have been deleted, leaving a stale link.
            // We verify the post actually exists and is published.
            // We also check for actual content — Polylang auto-creates empty stub
            // posts when a language subdomain/directory is configured (e.g. en.webclassmate.com),
            // those stubs are 'publish' but have no title or content and must be re-translated.
            if ($trans_id) {
                $trans_post = get_post($trans_id);
                if ($trans_post && $trans_post->post_status === 'publish') {
                    $has_content = !empty(trim($trans_post->post_content))
                        || !empty(trim($trans_post->post_title));
                    if ($has_content) {
                        continue; // genuinely translated — skip this language
                    }
                // Falls through: stub post exists but is empty → treat as untranslated
                }
            }
...
function plt_save_translation(WP_REST_Request $request)
{
    ...
    // We use MySQL's GET_LOCK() which is a true cross-process mutex.
    // Unlike transients (which use per-process object cache and are NOT
    // visible to concurrent PHP workers), GET_LOCK() blocks at the
    // database level until released, guaranteeing sequential access.
    global $wpdb;
    $lock_name = 'plt_post_' . $source_id;
    $lock_timeout = 10;
    ...
    try {
        // Flush object cache so we read fresh from the DB
...
function plt_get_untranslated_terms(WP_REST_Request $request)
{
    ...
    $term_ids = get_terms([
        'taxonomy' => $taxonomy,
        'hide_empty' => false,
        'lang' => '', // Fetch terms in all languages
        'orderby' => 'term_id',
        'order' => 'DESC', // Search newest first
        'fields' => 'ids',
    ]);
    ...
    foreach ($term_ids as $tid) {
        ...
        // Find the oldest term in the translation group to act as the single source
        $min_id = $tid;
        if (!empty($translations)) {
            $valid_ids = array_filter(array_values($translations));
            if (!empty($valid_ids)) {
        ...
        // If this term is not the oldest in its translation group, skip it to prevent duplicates
        if ($tid != $min_id) {
            continue;
        }

        $source_lang = pll_get_term_language($tid);
        if (empty($source_lang))
            continue;
...
function plt_save_term_translation(WP_REST_Request $request)
{
    ...
    try {
        // Flush the object cache for this term so we read fresh from the DB,
        // not a stale in-memory cached value from earlier in this request.
        clean_term_cache($source_id, $taxonomy);
        wp_cache_flush();
...
function plt_unique_term_slug(string $slug, string $taxonomy, int $exclude_term_id, string $lang_suffix): string
{
    ...
    if ($exclude_term_id && (int)$existing->term_id === $exclude_term_id)
        return $slug; // it's ours
    ...
    if (!$existing2)
        return $candidate;
    if ($exclude_term_id && (int)$existing2->term_id === $exclude_term_id)
        return $candidate;
