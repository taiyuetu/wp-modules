...
function plt_get_config()
{
    ...
    foreach ($post_types as $pt) {
        if (function_exists('pll_is_translated_post_type') && pll_is_translated_post_type($pt)) {
            $translatable_post_types[] = $pt;
        }
    }
    ...
    foreach ($taxonomies as $tax) {
        if (function_exists('pll_is_translated_taxonomy') && pll_is_translated_taxonomy($tax)) {
            $translatable_taxonomies[] = $tax;
        }
    }

    return rest_ensure_response([
        'languages' => $languages,
        'post_types' => $translatable_post_types,
        'taxonomies' => $translatable_taxonomies,
    ]);
}
...
function plt_get_untranslated_posts(WP_REST_Request $request)
{
    ...
    // If target language is the default language, we skip translation entirely
    if ($target_lang && $target_lang === $default_lang) {
        return rest_ensure_response([
            'total' => 0,
            'pages' => 1,
            'page' => $page,
    ...
    $post_ids = get_posts([
        ...
        'post_status' => 'publish',
        'posts_per_page' => -1,
        'lang' => $default_lang, // Fetch posts only in the default language
        'orderby' => 'ID',
    ...
    foreach ($post_ids as $pid) {
        ...
        if (!empty($missing_langs)) {
            ...
            if ($per_page > 0 && count($results) >= $per_page) {
                break;
            }
        }
    }
...
function plt_get_untranslated_terms(WP_REST_Request $request)
{
    ...
    $term_ids = get_terms([
        'taxonomy' => $taxonomy,
        'hide_empty' => false,
        'lang' => $default_lang, // Fetch terms only in the default language
        'orderby' => 'term_id',
